...
//list the book titles in reverse alphabetical order
const titles = books
    .map(book => book.title)

const reverseAlphabetOrd = [...titles].sort((a, b) => b.localeCompare(a)); //[...titles] / tiltes.slice() copy

console.log(titles);
console.log(reverseAlphabetOrd); 
 
//list only the the books published by Harper Collins and sort them by the year of publishing (newer first)
const HarperCollinsBooks = books.filter(book => book.publisher === 'Harper Collins')
    .sort((a, b) => (b.year - a.year)); //(b.year > a.year ? 1 : -1)

console.log(HarperCollinsBooks);

//list all the books containing companies or people in the title
const substringBooks = books
    .filter(function substrBooks(book) {
        return book.title.includes('companies') || book.title.includes('people');
    });

console.log(substringBooks);

//with function
function getTitlesReversOrder(books) {
    console.log(
        books.map(book => book.title).sort((a, b) => b.localeCompare(a))
    );
}

getTitlesReversOrder(books);
